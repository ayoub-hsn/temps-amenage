...
class FiliereController extends Controller
{
    ...
    public function indexBachelier(){
        $etablissement = Etablissement::where('responsable_id',Auth::id())->first();
        ...
        ->leftJoin('bacheliers', function ($join) {
            if (auth()->user() && auth()->user()->etablissement->multiple_choix_filiere_passerelle == 1) {
                $join->on('bacheliers.filiere_choix_1', '=', 'filieres.id')
        ...
        ->selectRaw('COUNT(bacheliers.id) as students_count')
        ->get();
        return view('admin-etab.filiere.indexBachelier',compact('etablissement','filieres'));
    }
    ...
    public function store(Request $request)
    {
        ...
        switch ($request->type) {
            ...
            case 2:
                ...
                break;
            case 3:
                return redirect()->route('admin-etab.filiere.bachelier.index')->with('message','Filiere est ajouté avec succée');
                break;
            default:
                return redirect()->route('admin-etab.filiere.licencexcellence.index')->with('message','Filiere est ajouté avec succée');
                break;
    ...
    public function showStudentsBacheliers(Filiere $filiere,Request $request){
        ...
        return view('admin-etab.etudiant.indexBachelierStudents',compact('etablissement','filiere'));
    }
