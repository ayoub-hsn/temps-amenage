	...
	<div class="form-container" v-if="!successForm">
	  ...
	  <div class="form-body">
		...
		<form @submit.prevent="nextStep" enctype="multipart/form-data">
		  ...
		  <div v-if="currentStep === 1">
				...
				<div class="form-group" dir="ltr">
					...
					<select
						id="handicap"
						v-model="etudiant.handicap"
						:class="{ 'input-error': errors.handicap }"
						@change="validateStep('handicap')"
						@focus="currentField = 'handicap'"
					>
						<option value="" disabled>Veuillez choisir</option>
						<option value="non">Non</option>
						<option value="oui">Oui</option>
					</select>
					<span v-if="errors.handicap" class="error-icon">✖ {{ errors.handicap }}</span>
				 </div>
		  ...
		  <div v-if="currentStep === 2">
			<fieldset>
				...
				<div class="form-group" style="margin-top: 20px;" dir="ltr">
					<label for="serie">Serie de BAC</label>
					<select
						id="serie"
						v-model="etudiant.serie"
						:class="{ 'input-error': errors.serie }"
						@change="validateStep('serie')"
						@focus="currentField = 'serie'"
					>
						<option value="" disabled>SERIE DE BAC</option>
						<option v-for="s in serie_bac" :key="s.id" :value=s.id> {{ s.nom }}</option>
				...
				<div class="form-group" dir="ltr">
					<label for="mention_bac">Mention de BAC</label>
					<select
						id="mention_bac"
						v-model="etudiant.mention_bac"
						:class="{ 'input-error': errors.mention_bac }"
						@change="validateStep('mention_bac')"
						@focus="currentField = 'mention_bac'"
					>
					...
					<span v-if="errors.mention_bac" class="error-icon">✖ {{ errors.mention_bac }}</span>
				 </div>
				 <div class="form-group" dir="ltr">
					<label for="notebac" >Moyenne de BAC</label>
					<input
						type="text"
						id="notebac"
						v-model="etudiant.notebac"
						placeholder="Moyenne de BAC"
						:class="{ 'input-error': errors.notebac }"
						@input="validateStep('notebac')"
						@focus="currentField = 'notebac'"

					/>
					<span v-if="errors.notebac" class="error-icon" >✖ {{ errors.notebac }}</span>
				</div>
			</fieldset>
		...
		<div v-if="currentStep === 3">
			...
			<div class="form-group student-image-upload" v-if="etablissement.show_photo_input_bac_acceeouvert">
			...
			<div v-if="etablissement.show_cin_input_bac_acceeouvert" class="form-group" style="margin-top: 20px;">
			...
			<div v-if="etablissement.show_bac_input_bac_acceeouvert" class="form-group" style="margin-top: 20px;">
				<!-- File Upload Field for Student Image -->
				<label class="file-label">Baccalauréat</label>
				<div class="file-upload-wrapper">
					<input
					type="file"
					id="path_bac"
			...
			<div class="form-group student-pdf-upload" v-if="etablissement.show_cv_input_bac_acceeouvert">
            ...
            <div style="text-align: center;" v-if="!etablissement.show_photo_input_bac_acceeouvert && !etablissement.show_cin_input_bac_acceeouvert && !etablissement.show_bac_input_bac_acceeouvert && !etablissement.show_cv_input_bac_acceeouvert">
                <h4>Aucun document à importer. Vous pouvez continuer à l'étape suivante.</h4>
            </div>
		...
		<div v-if="currentStep === 4">
			<div class="form-groupp">
				<label class="section-title">Votre Choix</label>

				<div class="filiere-container">
					<div class="filiere-card" @click="toggleSelection(filiere.id)">
						<input
							type="radio"
						...
						/>
						<label :for="'filiere_' + filiere.id" class="filiere-content">
							<div class="filiere-title">{{ filiere.nom_abrv }}</div>
							<div class="filiere-name">{{ filiere.nom_complet }}</div>

							<a v-if="filiere.document" :href="appurl + filiere.document" target="_blank" class="description-btn">
								<i class="fas fa-file-alt"></i> Voir la description
							</a>

							<div v-if="etudiant.filiere === filiere.id" class="selected-icon">
								✔ Sélectionné
							</div>
						</label>
					</div>
	...
	<div class="success-container" v-else>
		<div class="success-card">
			...
			<div class="success-info">
				<p>Un <strong>compte étudiant</strong> a été créé.</p>
			...
			<div class="success-credentials">
				...
				<p><strong>Mot de passe :</strong>
					...
					<span class="password-box" :class="{ 'blurred': !showPassword }">
						...
						<span v-if="showPassword">{{ etudiant.CIN }}-{{ etudiant.nom }}-{{ etudiant.prenom }}</span>
						<span v-else>********************************</span>
					</span>
...
<script>
  ...
  export default {
	props: ['appurl','cin','datenais','etablissement','filiere'],
	components:{
		ErrorPopup
	},
	data() {
	...
	watch: {
		...
		"etudiant.handicap"(newValue) {
		if (this.currentField === "handicap") {
			this.validateStep("handicap");
		}
		},
		...
		"etudiant.mention_bac"(newValue) {
		if (this.currentField === "mention_bac") {
			this.validateStep("mention_bac");
		}
		},
		"etudiant.notebac"(newValue) {
		if (this.currentField === "notebac") {
			this.validateStep("notebac");
		}
		},
	...
	methods: {	
		...
		validateStep(field) {
			if(this.currentStep == 1){
				...
				case "handicap":
					if (!this.etudiant.handicap) {
					this.errors.handicap = "Le champs est requis.";
					} else {
					this.errors.handicap = null;
					}
					break;
			...
			if(this.currentStep == 2){
				...
				switch (field) {
					case "serie":
						...
						this.errors.serie = "Série de bac est requis.";
						} else {
					...
					case "mention_bac":
						if (!this.etudiant.mention_bac) {
						this.errors.mention_bac = "Mention de bac est requis.";
						} else {
						this.errors.mention_bac = null;
						}
						break;

					case "notebac":
						if (!this.etudiant.notebac) {
						this.errors.notebac = "La note de bac est requise.";
						} else if (!/^(?:\d|1\d|20)(?:\.\d{1,2})?$/.test(this.etudiant.notebac)) {
						this.errors.notebac = "La note de bac doit être un nombre entre 0 et 20, avec un maximum de deux chiffres après la virgule.";
						} else {
						this.errors.notebac = null;
						}
						break;
		...
		validateStepp() {
			...
			if (this.currentStep == 3) {
				...
				if (this.etablissement.show_photo_input_bac_acceeouvert) {
					fileFields.push("path_photo");
				}

				if (this.etablissement.show_cin_input_bac_acceeouvert) {
					fileFields.push("path_cin");
				}

                if (this.etablissement.show_bac_input_bac_acceeouvert) {
				...
				if(this.etablissement.show_cv_input_bac_acceeouvert){
		...
		async submitForm() {
			...
			try {
				...
				// Send form data
				const response = await axios.post("/api/submit/form/bac/acceeouvert", formData, {
					headers: {
	...
	mounted() {
        ...
        this.etudiant.datenais = this.datenais
