<template>
	...
	<div class="form-container" v-if="!successForm">
	  ...
	  <div class="form-header">
		...
		<h1 class="form-title">Préinscription Licences (Accès S1)</h1>
		  ...
		  <div v-if="currentStep === 2">
			<fieldset>
                ...
                <div class="form-group">
					<label for="note">Moyenne du Bac</label>
					<input
						type="text"
						id="note"
						v-model="etudiant.moyenne_bac"
						placeholder="Entrez une note (ex: 16.50, 14)"
						@input="validateStep('moyenne_bac')"
						@focus="currentField = 'moyenne_bac'"
						:class="{ 'input-error': errors.moyenne_bac }"
					/>
					<span v-if="errors.moyenne_bac" class="error-icon">✖ {{ errors.moyenne_bac }}</span>
				</div>
			</fieldset>
	...
	methods: {
		...
		async submitForm() {
			...
			try {
				...
				// Send form data
				const response = await axios.post("/api/submit/form/bachelier", formData, {
					headers: {
