...
<script>
    ...
    $(document).ready(function () {
        $('#etudiant-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin-etab.master.candidat.index') }}",
            columns: [
                {
                    data: null,
                    name: 'nom',
                ...
                //     searchable: false,
                //     render: function (data, type, row) {
                //         return '<a href="/admin-etab/master/candidats/' + data + '" class="btn btn-info btn-sm mr-1">Afficher</a>' +
                //             '<a href="/admin-etab/master/candidats/' + data + '/edit" class="btn btn-warning btn-sm mr-1">Modifier</a>' +
                //             '<button type="button" class="btn btn-danger btn-sm mr-1" onclick="annulerConfirmation(' + data + ')">Annuler Confirmation</button>' +
                //             '<a href="/admin-etab/master/candidats/' + data + '/telechargerRecu" class="btn btn-success btn-sm">Télécharger Reçu</a>';
                //     }
                // }
            ],
            ...
            drawCallback: function () {
                $('#etudiant-table_paginate .pagination li').each(function() {
                    ...
                    var href = "{{ route('admin-etab.master.candidat.index') }}?page=" + pageNum;
    ...
    function annulerConfirmation(candidat){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                    ...
                    var url = '{{ route("admin-etab.master.candidat.annulerConfirmation", ":candidat") }}';
