...
<div class="application-container">
    <div class="application-header">
        <h1 class="application-title">Postuler</h1>
        <p class="application-subtitle">Merci de remplir le formulaire ci-dessous pour postuler à ce programme.</p>
    </div>
    @if (Session::has('error_student_exist'))
        <div class="alert alert-danger">{{ Session::get('error_student_exist') }}</div>
    @endif
    <form action="{{ route('bacheliers.welcomeBacAcceOuvert.apply',['etablissement' => $etablissement->id]) }}" method="POST" class="application-form">
        @csrf
        ...
        <div class="form-group">
            <label for="datenais" class="form-label">Date Naissance <span class="required">*</span></label>
            <input type="date" id="datenais" name="datenais" class="form-input {{ $errors->has('datenais') ? 'is-invalid' : '' }}">
        ...
        <div class="form-group">
            ...
            <select name="province" id="province" class="form-input {{ $errors->has('province') ? 'is-invalid' : '' }}">
