            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header" style="justify-content: space-between;">
                        <h4>Liste des étudiants - Master - {{ $filiere->nom_abrv }}</h4>
                    </div>
                    <div class="card-body">
...
<script>
    ...
    $(document).ready(function () {
        $('#etudiant-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('sup-admin.payment.master.filiere.students',['filiere' => $filiere->id]) }}",
            columns: [
                {
                    data: null,
                    name: 'nom',
                ...
                { data: 'type_master', name: 'type_master' },
                ...
                { data: 'montant_detecter', name: 'montant_detecter' },
                {
                    ...
                    render: function (data, type, row) {
                        let btnShow = 
                            '<a href="/sup-admin/paiement/master/filiere/' + filiereId + '/student/' + data + '/show" class="btn btn-info btn-sm mr-1">Afficher</a>';
            ...
            drawCallback: function () {
                $('#etudiant-table_paginate .pagination li').each(function() {
                    ...
                    var href = "{{ route('sup-admin.payment.master.filiere.students',['filiere' => $filiere->id]) }}?page=" + pageNum;
...
<script>
    $(document).on('click', '.btn-check', function () {
        ...
        $.ajax({
            url: '/sup-admin/paiement/' + paymentId + '/check/master',
            type: 'POST',
            headers: {
