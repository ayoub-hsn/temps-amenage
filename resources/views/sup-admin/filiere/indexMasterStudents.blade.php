...
<script>
    ...
    $(document).ready(function () {
        $('#etudiant-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('sup-admin.etablissement.filiere.master.etudiants',['filiere' => $filiere->id]) }}",
            columns: [
                { 
                    data: null,
                    name: 'nom',
                ...
                { data: 'verif', name: 'verif' },
                {
                    ...
                    searchable: false,
                    render: function (data, type, row) {
                        return '<a href="/sup-admin/filiere/' + filiereId + '/master/etudiant/' + data + '" class="btn btn-info btn-sm mr-1">Afficher</a>' + 
                        '<a href="/sup-admin/master/candidats/' + data + '/edit" class="btn btn-warning btn-sm mr-1">Modifier</a>' ;
                    }
                }
            ],
            ...
            drawCallback: function () {
                $('#etudiant-table_paginate .pagination li').each(function() {
                    ...
                    var href = "{{ route('sup-admin.etablissement.filiere.master.etudiants',['filiere' => $filiere->id]) }}?page=" + pageNum;
